  ...
  try {
    ...
    if (req.body.nextBlock) {
      const { error } = Joi.number()
        .integer()
        .greater(0)
        .validate(Number(req.body.nextBlock));
      if (error) {
        return res.status(400).json({
          success: false,
    ...
    if (isLastBlock && Boolean(Number(isLastBlock))) {
      ...
      const stats = fs.statSync(
        path.join(__dirname, "../videos", `${upload_url}.webm`)
      );
      video.last_chunk += 1;
...
// https://prod.liveshare.vsengsaas.visualstudio.com/join?5AF1ACFCDEBC796F9C5F87963A19C760A2FF
